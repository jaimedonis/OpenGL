...
void createRenderObjects(android_app *pApp, Renderer* renderer)
{
    ...
    //auto world_transform = glm::identity<glm::mat4>();
    ...
    auto camera_position = scene_center + glm::vec3(0.0f, 6.0f, scene_radius * 1.8f);
    auto main_light_position = glm::vec3(0.5f, 20.0f, 10.0f);

    std::unique_ptr<CameraBaseNode> app_camera = std::make_unique<PerspectiveCamera>(45.0f, 1024, 768, 0.01f, 500.0f);
    app_camera->LookAt(camera_position, scene_center);
    scene->AddCamera(app_camera);

    SceneLight scene_light1;
    scene_light1.light_type = LightType::Point;
    scene_light1.light_position = main_light_position;
    scene_light1.light_color = glm::vec3(1.0);
    scene->AddLight(scene_light1);

    SceneLight scene_light2;
    scene_light2.light_type = LightType::Directional;
    scene_light2.light_position = glm::vec3(main_light_position.x-10, main_light_position.y, main_light_position.z);
    scene_light2.light_color = glm::vec3(0.1, 0.1, 0);
